    ...
    return (
        <div>
            <ul>{book.key_ideas.map((idea) =>
                <li className='relative flex flex-col justify-center w-100% min-h-[12rem] rounded-md text-center bg-white my-6 shadow-md p-4 hover:scale-[1.02] ease-in duration-300'>
                    <h3 className='text-2xl font-bold italic'>{idea.title}</h3>
                    <p className='text-lg mt-4'>{idea.description}</p>
                    <AIToolTip className='absolute right-0 top-0 m-4' />
                </li>
            )}
            </ul>
        </div>
        ...
        <div>
            <ul className='flex items-center justify-center text-center m-auto gap-8'>
                {titles.map((title, idx) =>
                    <li className={`min-w-[12rem] text-lg font-bold ${idx === index ? "opacity-100" : "opacity-70"} bg-[#373738] text-white rounded-lg p-2 cursor-pointer hover:scale-105 duration-150`} onClick={() => setIndex(idx)}>{title}</li>
                )}
            </ul>
            {bodies[index]}
        </div>
...
export default function BookPage({ book }) {
    return (
        <main className={`bg-gradient-to-b from-[#76a4ca] to-[24rem] to-[#e7e7e9] w-full h-full m-auto pt-4`}>
            ...
            <div className='flex pl-32 py-12 gap-16'>
                <div className='shrink-0'>
                    ...
                    <Link href={book.amazon_url}>
                        <h2 className='text-2xl font-bold text-center m-2 text-[#158582]'>Support the Author <br /> Buy it!</h2>
                    </Link>
                </div>
                <div className='grow'>
                    ...
                    <ul className='flex gap-4 mt-2'>{book.topics.map((topic) =>
                        <li className='bg-[#F4E6CB] py-1 px-2 rounded-lg font-medium'>{topic}</li>)}
                    </ul>
                ...
                <div className='basis-1/4'>
                    ...
                    {book.similar_books.map((sb) =>
                        <Link href={`book/${sb.slug}`}>
                            <div className='p-2 mx-2 my-4 bg-white rounded-lg hover:scale-105 ease-in duration-100'>
                                <div className='relative flex flex-col w-[12rem] h-[18rem] m-auto text-center'>
